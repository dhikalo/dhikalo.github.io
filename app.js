...
function initUI() {
    ...
    safeOn('inputPlanImage', 'change', (e) => {
        ...
        img.onload = () => {
            ...
            L.imageOverlay(url, boundsLL).addTo(map);
            map.fitBounds(boundsLL);
            showToast("Offline-Plan geladen.");
        };
        img.src = url;
    });
    ...
    safeOn('btnClearShapes', 'click', () => { if(AppState.drawItems) AppState.drawItems.clearLayers(); showToast("Auswahlbereich gelöscht"); });
...
function initMap() {
    ...
    window.drawControl = new L.Control.Draw({
        edit: false, // COMPLETELY REMOVED THE EDIT/DELETE TOOLBAR
        draw: { 
...
async function triggerAusschnitt(targetLayer) {
    ...
    try {
        ...
        const canvas = await html2canvas(mapEl, {
            useCORS: true,
            allowTaint: true,
            scale: (window.innerWidth < 1200) ? 1 : (window.devicePixelRatio || 1), // Cap scale on iPad
            logging: false,
            backgroundColor: '#111'
        });
        ...
        TABLE_STRUCTURE.forEach(g => {
            ...
            groupCols.forEach((col, colIdx) => {
                ...
                if(col.toLowerCase().includes('anhang')) {
                    td.style.display = 'flex'; td.style.gap = '6px'; td.style.justifyContent = 'center';
                    ...
                    td.style.height = '32px'; // Match standard row height
                    if(originalRow[col]) {
                        const b = document.createElement('button'); b.className='btn-modern'; b.innerHTML='👁️';
                        b.style.padding = '4px 8px'; b.onclick = () => openImagePreview(originalRow[col], "Vorschau");
                        td.appendChild(b);
                        const delBtn = document.createElement('button'); delBtn.className='btn-modern btn-danger'; delBtn.innerHTML='✕';
    ...
    try {
        ...
        for (let i = 0; i < AppState.data.length; i++) {
            ...
            depthsToExport.forEach((d, depthIndex) => {
                ...
                if (imgData && imgData.startsWith('data:image')) {
                    ...
                    worksheet.addImage(imgId, {
                        tl: { col: imgColIndex, row: 7 + i, nativeColOff: 30, nativeRowOff: 30 },
                        ext: { width: 240, height: 200 }
                    });
                }
            });
            ...
            if (gesAnhangImgData && gesAnhangImgData.startsWith('data:image')) {
                ...
                worksheet.addImage(imgId, {
                    tl: { col: gesAnhangColIdx, row: 7 + i, nativeColOff: 30, nativeRowOff: 30 },
                    ext: { width: 240, height: 200 }
                });
            }
        }
